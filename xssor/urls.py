...
from xssor.index.views import index, cmd_create, cmd, \
                            probe_create, probe_js, probe_txt, probe_status, probe_delete, haha
...
urlpatterns = [
    ...
    url(r'^probe/(?P<pid>\w+)/(?P<probeName>\w+)/.*.js/?$', probe_js),
    url(r'^probe/(?P<pid>\w+)/(?P<probeName>\w+)/.*.txt/?$', probe_txt),
    url(r'^probe/status/?$', probe_status),
    url(r'^probe/delete/?$', probe_delete),
    url(r'haha.html', haha),
]
